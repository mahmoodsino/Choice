    ...
    <li className="">
      ...
      >
        <Link href={`/products?category=${node.id}`}>
          <a
            className={`block w-full text-left hover:bg-yellow-950/25 px-2 py-3 text-xs border-b text-gray-1200 ${
              selectedParentId === node.id && "bg-yellow-950/25"
            }`}
          >
            {node.name}
          </a>
        ...
        {hasChild && selectedParentId === node.id && (
          <div className=" bg-white ">
            <ul
              className={` absolute  text-black bg-white  w-64  text-left  border border-t-2 border-t-yellow-950  -top-[2px] left-[95.5%]  z-50`}
            >
              <HomeTree data={node} />
            </ul>
          </div>
